  ...
  return (
    <div className="max-w-3xl mx-auto pb-12">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link to="/recipes" className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-800 dark:hover:text-gray-200 transition-colors active:scale-90">
          <ArrowLeft size={18} />
        </Link>
        <div className="flex-1" />
        <button
          onClick={toggleFavorite}
          className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors active:scale-90"
        >
          <Heart size={20} className={recipe.is_favorited ? 'fill-red-500 text-red-500' : 'text-gray-300 hover:text-red-400'} />
        </button>
        {isOwner && (
          <>
            <Link to={`/recipes/${id}/edit`} className="p-2 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors active:scale-90">
              <Edit size={18} />
            </Link>
            <button onClick={() => setShowDeleteDialog(true)} className="p-2 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors active:scale-90">
              <Trash2 size={18} />
            </button>
          </>
        )}
      ...
      {recipe.image_url && !imgError ? (
        <div className="relative mb-6 rounded-2xl overflow-hidden shadow-card-hover">
          <img
      ...
      {(recipe.prep_time || recipe.cook_time || recipe.servings) && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          {recipe.prep_time ? (
            <div className="bg-white dark:bg-gray-800 rounded-xl p-3 text-center shadow-card border border-gray-100 dark:border-gray-700">
      ...
      {/* Description */}
      {recipe.description && (
        <p className="text-gray-600 dark:text-gray-300 mb-6 leading-relaxed text-sm bg-white dark:bg-gray-800 rounded-xl p-4 shadow-card border border-gray-100 dark:border-gray-700">
          {recipe.description}
        </p>
      )}
        ...
        <div>
          <div className="flex bg-gray-100 dark:bg-gray-800 rounded-xl p-1 mb-5">
            <button
              ...
              className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors active:scale-[0.97] ${
                tab === 'ingredients'
                  ? 'bg-white dark:bg-gray-700 text-orange-500 shadow-card'
            ...
            <div className="space-y-3">
              {!hasCats ? (
                recipe.ingredients.map(ing => (
                  <div key={ing.id} className="flex items-center gap-3 bg-white dark:bg-gray-800 rounded-xl px-4 py-3 shadow-card border border-gray-100 dark:border-gray-700">
            ...
            <div className="space-y-3">
              {recipe.steps.map(s => {
                const stepIngs = (s.ingredientIds || [])
                  ...
                  .filter(Boolean);
                return (
                  <div key={s.id} className="bg-white dark:bg-gray-800 rounded-2xl shadow-card border border-gray-100 dark:border-gray-700 p-4 flex gap-4">
      ...
      <ConfirmDialog
        ...
        title="Delete Recipe"
        description={<>Are you sure you want to delete <span className="font-semibold text-gray-700 dark:text-gray-200">"{recipe.title}"</span>? This cannot be undone.</>}
        icon={Trash2}
        loading={deleting}
